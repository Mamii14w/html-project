     ...
     <td >
                                 ...
                                 if (isset($_POST['submit']))
                    {
                                    ...
                                    $d_name=$_POST['dep'];
                                     ...
                                     $sql="insert into department values('','$prog_id','$d_name')";
                        
                        
                            if(mysqli_query($con,$sql))
                                    {

                            $_SESSION['msg']="Department Added Succesfuly";
                            header("location:Add_department.php");
                                    }
                                    else
                                    {
                                        $_SESSION['msg']="Department Does Not Added".mysqli_error($con);
                                        header("location:Add_department.php");
                ...
                <table id="tr" style="border:1px solid black; border-radius:20px;margin-left:-0px;margin-top:0px;box-shadow:10px 20px 10px black;" width="500" height="400px">
                <tr bgcolor="lightblue"><td colspan=2 align=center><font color=black> Department Adding Form
                </td></tr>
                
               
                
                <tr><td>Department Name :</td><td><input id="td" type="text" name="dep" placeholder='department name'required pattern=[a-zA-z. ]{1,40} required=""oninvalid="this.setCustomValidity('Please enter valid department name')"oninput="setCustomValidity('')"></td></tr>
                 <tr>
                           <td></td>
                           <td>
                            <?php    
                            $result = mysqli_query($con,"SELECT *from programs WHERE pro_id='1'");
                            $row= mysqli_fetch_array($result)
                            ?>
                                <input type="hidden" value='<?php echo $row['pro_id']?>'  name="prog_id">
