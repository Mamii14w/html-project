...
<?php 
           ...
           if(isset($_SESSION["username"])) 
        {
		 ...
		 if(!preg_match('/^[a-zA-Z]+$/',$first_name) || empty($first_name) || preg_match( '/[\'\/~`\!@#\$%\^&\*\(\)_\-\+=\{\}\[\]\|;:"\<\>,\.\?\\\]/' , $first_name) || preg_match( '/[\s]+/',$first_name))
			      ...
			      $sql="insert into user values('','$first_name','$last_name','$sex','$age','$phone','$email')";
				    ...
				    $password = substr(str_shuffle($string),0,6);
                     ...
                     if ($last_id) {
                     	$username = "HRU/DU".$last_id."/".date('y');
					...
					$query2="insert into user_account values('','$last_id','$username','$password','$role','1')";
